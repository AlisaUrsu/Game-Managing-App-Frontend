...
interface AddGameToListModalProps {
    game: Game;
    onCancel: () => void;
    onAddGame: (status: string, review?: string, rating?: number) => void;
}

const AddGameToListModal = ({ game, onCancel, onAddGame }: AddGameToListModalProps) => {
    ...
    const onSubmit = (data: GameListInput) => {
        console.log(rating);
        onAddGame(data.status, data.review, rating);
    };
